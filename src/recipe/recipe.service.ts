import { Injectable, BadRequestException, NotFoundException, HttpStatus, Logger, UnauthorizedException, HttpException } from '@nestjs/common';
...
export class RecipeService {
    ...
    private toResponseObject(recipe: RecipeEntity): RecipeRO{
        const responseObject: any = {...recipe, author: recipe.author.toResponseObject() };
    ...
    async bookmark (id: string, userId: string) {
        ...
        if (user.bookmarks.filter(bookmark => bookmark.id === recipe.id).length < 1) {
    ...
    async unbookmark (id: string, userId: string) {
        ...
        if (user.bookmarks.filter(bookmark => bookmark.id === recipe.id).length > 0) {
